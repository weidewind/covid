...
# start=`date +%s`
# parallel python -u country_stat.py --tree ${all_states_file}.newick --states ${all_states_file}.probs --countries $leaf_states_file --entry_nodes_file ${translin_file}.entries --max_distance {} --output ${country_stat_file}.{} >${country_stat_file}.logger.{} ::: 0 1 2 3 4
# end=`date +%s`
# runtime=$( echo "$end - $start" | bc -l )
