...
start=`date +%s`
Rscript analyze_dates.R --pangolined_file $rus_pangolined --cluster_dates_file ${output_folder}/cluster_strains.dates_stats --entry_strains_file ${translin_file} --entry_dates_file ${translin_file}.dates_stats --important_lineages $important_lineages --output_folder $pangolin_output_folder --tag $gentag
end=`date +%s`
runtime=$( echo "$end - $start" | bc -l )
