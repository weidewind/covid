...
def short_country(country):
	if country == "Russia" or country == "unknown":
		return(country)
	else:
		return("foreign")
		...
		for lin in row["lineage_stat"].split(";"):
			...
			for ilin in ilins:
				if lin == ilin or (len(lin) > len(ilin) and lin[:len(ilin)+1] == ilin + "."):
					...
					entry_to_ipangos[row["entry"]] = entry_to_ipangos.get(row["entry"], "")  + ilin + "="
...
for entry in entries:
	...
	stripped_nodes_country_dict = {} 
	stripped_nodes_rus_dict = {} 
	stripped_nodenames = [l.name for l in t.get_leaves() if re.match(r"^[0-9]", l.name)]
	print("stripped " + ",".join(stripped_nodenames))
	for nname in stripped_nodenames:
		strnode = tnode.search_nodes(name=nname)[0]
		strleaves = [l.name for l in strnode.get_leaves()]
		strn_countries = [short_country(countries_dict.get(sl, "unknown")) for sl in strleaves]
	#	strn_rus = ""
		for sl in strleaves:
	#		if countries_dict.get(sl, "unknown") == "Russia":
	#			strn_rus = strn_rus + ";" + sl
			if sl in duplicates:
				strn_countries.extend([short_country(countries_dict.get(d, "unknown")) for d in duplicates[sl]])
		stripped_nodes_country_dict[nname] = countries_collection_to_str(strn_countries)
	#	stripped_nodes_rus_dict[nname] = strn_rus
	...
	def multiple_country_layout(node):
		...
		elif node.is_leaf() and not node.name == entry_node.name:
			faces.add_face_to_node(faces.TextFace(stripped_nodes_country_dict[node.name]), node, column=0)
			#faces.add_face_to_node(faces.TextFace(stripped_nodes_rus_dict[node.name]), node, column=0)
	...
	ts.scale = 100000
	t.ladderize()
	t.render(options.output + "=" + entry_to_ipangos[entry] + "_entry_" + entry +  ".svg", tree_style=ts, dpi=300, w=10000, units="mm")
	t.render(options.output + "=" + entry_to_ipangos[entry] + "_entry_" + entry + ".pdf", tree_style=ts, dpi=300, w=10000, units="mm")
