...
rug_plot_selected<-function(dates, pngpath, value, mutations, foreign_mutations, districts = NULL){
		...
		p<-ggplot(dates) +  
			...
			geom_count(data = non_delta_foreign, aes(x=date, y=entry_sorted), color = "black", shape = 2, alpha = 0.5)+
			...
			if (is.null(districts)){
				...
				geom_count(data = delta_foreign, aes(x=date, y=entry_sorted, color=mutated), shape = 2, alpha = 0.5)
...
if(!is.null(opt$mutations_file) & !is.null(opt$foreign_mutations_file)){
	...
	if(is.null(opt$districts)){
		rug_plot_selected(dd, pngpath = paste(c(opt$output_folder, "/", opt$tag, "_corrected_grouped_mean_all_with_uncorr_median_foreign_date_all_delta_doublemut.png"),collapse=""), value= "corrected_grouped_mean_all",  mutations = mutations_df, foreign_mutations = f_mutations_df)
	}else{
		districts = read.table(opt$districts, header = F, sep = "\t")
		colnames(districts) = c("strain", "district")
		rug_plot_selected(dd, pngpath = paste(c(opt$output_folder, "/", opt$tag, "_corrected_grouped_mean_all_with_uncorr_median_foreign_date_all_delta_doublemut_districts.png"),collapse=""), value= "corrected_grouped_mean_all",  mutations = mutations_df, foreign_mutations = f_mutations_df,districts = districts )
	}
}else{
